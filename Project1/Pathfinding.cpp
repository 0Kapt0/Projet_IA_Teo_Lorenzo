    ...
    while (!openQueue.empty()) {
        ...
        for (auto& dir : directions) {
            ...
            if (neighborPos.x < 0 || neighborPos.x >= GRID_WIDTH || neighborPos.y < 0 || neighborPos.y >= GRID_HEIGHT)
                continue;
            if (!grid.getCell(neighborPos.x, neighborPos.y).walkable)  // Correction ici
                continue;

            if ((dir.x != 0 && dir.y != 0) &&
                (!grid.getCell(current->position.x, neighborPos.y).walkable || !grid.getCell(neighborPos.x, current->position.y).walkable))  // Correction ici
                continue;
